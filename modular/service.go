...
type BasicService interface {
	// OnInit 服务初始化阶段，将传入应用程序 Application 的引用，通常情况下需要将该引用进行保存，所有的其他服务均可以通过该引用获取到
	//  - 该阶段不应该依赖其他任何服务
...
// ActorService 标识模块化服务为 Actor 服务
type ActorService[S any] interface {
	// OnActorSystemInit ActorSystem 初始化阶段，将传入 ActorSystem 的引用
	OnActorSystemInit(system vivid.ActorSystem[S])
}
