...
// RegisterService 注冊一个基本服务
//   - Instance 是服务的实例类型，该类型必须是一个实现了 BasicService 接口的结构体指针类型，用于提供服务的具体实现
//   - Exposer 是服务的暴露接口，用于提供服务的对外接口
//
// 通过该函数注册的服务将会在应用程序启动时被实例化，并且在整个应用程序的生命周期中只存在一个实例
func RegisterService[Instance BasicService, Exposer any](application *Application) {
	tof := reflect.TypeOf(new(Instance)).Elem().Elem()
	vof := reflect.New(tof).Interface()
	service := vof.(BasicService)
	exposer := vof.(Exposer)
